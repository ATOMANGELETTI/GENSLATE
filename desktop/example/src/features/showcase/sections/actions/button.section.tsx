...
const VARIANTS: readonly { variant: ButtonVariant; label: string; hover: string; pressed: string }[] = [
  { variant: 'primary', label: 'Primary', hover: 'bg-accent-hover', pressed: 'bg-accent-active' },
  { variant: 'secondary', label: 'Secondary', hover: 'bg-control-hover', pressed: 'bg-control-pressed' },
  ...
  { variant: 'link', label: 'Link', hover: 'underline underline-offset-2', pressed: 'underline underline-offset-2' },
];
    ...
    <>
      ...
      <StateMatrix
        ...
        rows={VARIANTS.map(({ variant, label, hover, pressed }) => ({
          label,
          cells: [
            <Button key="rest" variant={variant}>{label}</Button>,
            <Button key="hover" variant={variant} className={hover}>{label}</Button>,
            <Button key="pressed" variant={variant} className={pressed}>{label}</Button>,
            <Button key="focus" variant={variant} className={FOCUS}>{label}</Button>,
            <Button key="disabled" variant={variant} disabled>{label}</Button>,
            <Button key="loading" variant={variant} loading>{label}</Button>,
          ],
        }))}
      />
      ...
      <Specimen title="Icons, loading and width" stageClassName="flex-col items-start gap-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="primary" leadingIcon="codicon:cloud-upload" loading={saving} onClick={save}>
            Publish
          </Button>
      ...
      <PropsTable
        rows={[
          { name: 'variant', type: "'primary' | 'secondary' | 'ghost' | 'danger' | 'link'", default: "'secondary'", description: 'Visual weight.' },
          { name: 'size', type: "'xs' | 'sm' | 'md' | 'lg'", default: "'md'", description: 'Height 20 / 24 / 28 / 32.' },
          { name: 'loading', type: 'boolean', default: 'false', description: 'Spinner, keeps width, aria-busy, stays focusable.' },
          { name: 'leadingIcon · trailingIcon', type: 'CodiconRef | ReactNode', description: 'Icons sized to the button.' },
